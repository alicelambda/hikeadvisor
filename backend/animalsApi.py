...
def getAnimal () :
	...
	for i in range (0, len (states)) :
		...
		max_per_page = 4
		url = ("https://api.inaturalist.org/v1/observations?photos="
			+ has_photo
			+ "&lat="
			+ str(lat_list[i])
			+ "&lng="
		...
		for j in range (0, max_per_page) :
			try :
				...
				description_url = ("https://api.inaturalist.org/v1/taxa/"
					+ str(info_id)
					)
				...
				if (jsonRes["results"] == None or len (jsonRes["results"]) == 0 or jsonRes["results"][j]["taxon"] == None
					or jsonRes["results"][j]["taxon"]["default_photo"] == None) :
				    continue

				#print(info_scientificName)
				info_picURL = jsonRes["results"][j]["taxon"]["default_photo"]["medium_url"]
				info_commonName = jsonRes["results"][j]["taxon"]["preferred_common_name"]
